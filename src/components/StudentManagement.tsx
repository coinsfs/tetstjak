...
const StudentManagement: React.FC = () => {
  ...
  const getStartPosition = (page: number, limit: number) => {
    return (page - 1) * limit;
  };
  ...
  const fetchStudents = useCallback(async () => {
    ...
    try {
      ...
      const start = getStartPosition(currentPage, recordsPerPage);
      const response = await studentService.getStudents(token, start, recordsPerPage, filters);
      
      setStudents(response.data);
      // Gunakan recordsFiltered sebagai total data siswa
      setTotalRecords(response.recordsFiltered);
      setTotalPages(Math.ceil(response.recordsFiltered / recordsPerPage));
  ...
  const handleToggleStatus = useCallback(async (student: Student) => {
    ...
    try {
      ...
      await studentService.toggleStudentStatus(token, student._id, newStatus);
