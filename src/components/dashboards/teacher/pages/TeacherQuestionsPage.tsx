...
const TeacherQuestionsPage: React.FC = () => {
  ...
  const [submissionFilters, setSubmissionFilters] = useState<QuestionSubmissionFilters>({
    academic_period_id: '',
    search: '',
    purpose: '',
  ...
  }, [filters, submissionFilters, questionSource]);
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      if (questionSource === 'my_questions') {
        ...
        // This shouldn't be called anymore for submitted questions
        return;
      }
  ...
  const fetchSubmissions = async () => {
    ...
    try {
      ...
      if (error.message.includes('403') || error.message.includes('Forbidden')) {
        ...
        toast.error('Gagal memuat daftar soal yang disubmit');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmissionFilterChange = (key: keyof QuestionSubmissionFilters, value: any) => {
    setSubmissionFilters(prev => ({
      ...prev,
      [key]: value
    }));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
          {questionSource === 'my_questions' ? (
            ...
            <>
              ...
              <select
                value={submissionFilters.academic_period_id}
                onChange={(e) => handleSubmissionFilterChange('academic_period_id', e.target.value)}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition-colors text-sm"
              >
              ...
              <input
                type="text"
                placeholder="Tujuan..."
                value={submissionFilters.purpose}
                onChange={(e) => handleSubmissionFilterChange('purpose', e.target.value)}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition-colors text-sm"
              />

              {/* Question Type Filter */}
              <select
                value={submissionFilters.question_type || ''}
                onChange={(e) => handleSubmissionFilterChange('question_type', e.target.value || undefined)}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition-colors text-sm"
              >
                <option value="">Semua Tipe Soal</option>
                <option value="multiple_choice">Pilihan Ganda</option>
              ...
              {/* Difficulty Filter */}
              <select
                value={submissionFilters.difficulty || ''}
                onChange={(e) => handleSubmissionFilterChange('difficulty', e.target.value || undefined)}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition-colors text-sm"
              >
                <option value="">Semua Tingkat</option>
                <option value="easy">Mudah</option>
              ...
              <select
                ...
                onChange={(e) => handleSubmissionFilterChange('status', e.target.value || undefined)}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition-colors text-sm"
              >
        ...
        {((questionSource === 'my_questions' && (filters.search || filters.difficulty || filters.question_type)) ||
          (questionSource === 'submitted_questions' && (submissionFilters.search || submissionFilters.purpose || submissionFilters.question_type || submissionFilters.difficulty || submissionFilters.status))) && (
          <div className="mt-4 pt-4 border-t border-gray-200">
            <div className="flex items-center space-x-2 text-sm text-gray-600">
              <span className="font-medium">Filter aktif:</span>
              <div className="flex flex-wrap gap-2">
                {(questionSource === 'my_questions' ? filters.search : submissionFilters.search) && (
                  <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-md">
                    Pencarian: "{questionSource === 'my_questions' ? filters.search : submissionFilters.search}"
                  </span>
                )}
                {(questionSource === 'my_questions' ? filters.difficulty : submissionFilters.difficulty) && (
                  <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-md">
                    Tingkat: {getDifficultyLabel(questionSource === 'my_questions' ? filters.difficulty : submissionFilters.difficulty!)}
                  </span>
                )}
                {(questionSource === 'my_questions' ? filters.question_type : submissionFilters.question_type) && (
                  <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-md">
                    Tipe: {getTypeLabel(questionSource === 'my_questions' ? filters.question_type : submissionFilters.question_type!)}
                  </span>
                )}
                {questionSource === 'submitted_questions' && submissionFilters.purpose && (
                  <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-md">
                    Tujuan: "{submissionFilters.purpose}"
                  </span>
                )}
                {questionSource === 'submitted_questions' && submissionFilters.status && (
                  <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-md">
                    Status: {getStatusLabel(submissionFilters.status)}
                  </span>
                )}
              </div>
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600">
              {totalItems} soal ditemukan {questionSource === 'submitted_questions' ? '(soal yang disubmit)' : '(soal Anda)'}
            </div>
        ...
        {/* Content Area */}
        <div className="p-6">
          {questionSource === 'submitted_questions' && !isCoordinator ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-4">
                Anda bukan koordinator mata pelajaran. Hanya koordinator yang dapat melihat soal yang disubmit untuk direview.
              </p>
            </div>
          ) : loading ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                {((questionSource === 'my_questions' && (filters.search || filters.difficulty || filters.question_type)) ||
                  ...
                  : questionSource === 'my_questions' ? 'Belum ada soal' : 'Belum ada soal yang disubmit'
                }
              </h3>
              <p className="text-gray-600 mb-4">
                {((questionSource === 'my_questions' && (filters.search || filters.difficulty || filters.question_type)) ||
                        ...
                        {submissions.map((submission, index) => (
                          <tr key={submission._id} className="hover:bg-gray-50 transition-colors">
                            <td className="px-6 py-4">
                              <div className="max-w-xs">
                                <div className="text-sm font-medium text-gray-900 truncate">
                                  {submission.question_details.question_text.replace(/<[^>]*>/g, '')}
                                </div>
                                <div className="text-xs text-gray-500 mt-1">
                  ...
                  {questionSource === 'my_questions' ? (
                    ...
                    <div className="space-y-6">
                      {submissions.map((submission) => (
                        <div key={submission._id} className="border border-gray-200 rounded-lg p-6 bg-white hover:shadow-md transition-shadow">
                          <div className="flex items-start justify-between mb-4">
                            <div className="flex-1">
                              ...
                              <div className="text-sm text-gray-600 mb-2">
                                <strong>Tujuan:</strong> {submission.purpose}
                              </div>
                            </div>
                            <div className="flex items-center space-x-2">
                              <button
